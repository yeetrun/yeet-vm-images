...
workflow_ref="$repository/.github/workflows/build-firecracker-runtime.yml@refs/heads/main"
[ "${GITHUB_WORKFLOW_REF:-}" = "$workflow_ref" ] || fail "publisher requires the approved scheduled/manual workflow on main"
[ "${YEET_RUNTIME_WORKFLOW_REPOSITORY:-}" = "$repository" ] || fail "publisher requires the reviewed workflow repository"
[ "${YEET_RUNTIME_WORKFLOW_FILE_PATH:-}" = .github/workflows/build-firecracker-runtime.yml ] || fail "publisher requires the reviewed workflow file path"
[ "${YEET_RUNTIME_WORKFLOW_REF:-}" = "$workflow_ref" ] || fail "publisher requires the approved full workflow ref"
[ "${YEET_RUNTIME_WORKFLOW_SHA:-}" = "$target" ] || fail "workflow SHA does not match the exact target commit"
